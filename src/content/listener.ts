...
interface ContentEditableSession {
  type: "content";
  element: HTMLElement;
  range: Range;
}

type SlashSession = (TextSession | ContentEditableSession) & {
  pointerRect: DOMRect;
};

const HOST_CACHE = new Set<string>();
let allowAllHosts = true;
const globalContext = globalThis as typeof globalThis & {
  promptPaletteStore?: PromptPaletteStore;
  PromptPaletteUI?: {
    CommandPalette: PaletteConstructor;
  };
};
...
let palette: PaletteInstance | null = null;
let session: SlashSession | null = null;

const normalizeHost = (host: string): string => host.trim().toLowerCase();

const hydrateWhitelist = async (): Promise<void> => {
  if (!store) {
    return;
  }
  const entries = await store.listWhitelist();
  HOST_CACHE.clear();
  entries.forEach((entry: SiteWhitelistEntry) =>
    HOST_CACHE.add(normalizeHost(entry.hostname))
  );
  allowAllHosts = HOST_CACHE.size === 0;
};

const hydrateCommands = async (): Promise<void> => {
  if (!store) {
    return;
  }
...
const computeAnchorRect = (element: EditableElement): DOMRect => {
...
const startTextSession = (
  ...
  const previousChar =
    cursor >= 2 ? element.value.charAt(cursor - 2) : "";
  if (previousChar === "/") {
    resetSession();
    return;
  }
...
const startContentSession = (element: HTMLElement): void => {
  const selection = window.getSelection();
  ...
  const anchorText =
    anchorNode && anchorNode.textContent ? anchorNode.textContent : "";
  if (
...
const isSlashInputEvent = (event: InputEvent): boolean => {
...
const readQueryForSession = (current: SlashSession): string | null => {
...
const insertIntoContentEditable = (
  current: ContentEditableSession,
  content: string
): void => {
  const selection = window.getSelection();
  if (!selection || selection.rangeCount === 0) {
...
const handleStorageChange = (state: StorageShape): void => {
  commands = state.commands;
  HOST_CACHE.clear();
  state.siteWhitelist.forEach((entry: SiteWhitelistEntry) =>
    HOST_CACHE.add(normalizeHost(entry.hostname))
  );
  allowAllHosts = HOST_CACHE.size === 0;
};
